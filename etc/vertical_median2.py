    ...
    def forward(self, img, mask,vertical_size):
        ...
        for col in range(img.shape[1]): #720
            for ch in range(img.shape[2]): #3
                ...
                # print("col_data.shape", col_data.shape) #(480,)
                # col_data_filtered = median_filter(col_data, size=vertical_size) #size=3
                col_data_filtered = median_filter_custom(col_data, size=vertical_size) #size=3
                # 데이터를 한 칸 아래로 이동 (padding 사용)
                shifted_data = np.pad(col_data_filtered, (1, 0), mode='constant')[:-1]
                # 두 번째 필터 적용
                # col_data_filtered_twice = median_filter(shifted_data, size=3, mode='nearest')
                col_data_filtered_twice = median_filter_custom(shifted_data, size=3)
                # print("col_data_filtered.shape", col_data_filtered.shape) #(480,)
...
def process_images_in_folder_twice(folder_path,save_path, unimatch, verticalmedian, threshold, magnitude, vertical_size):
...
if __name__ == '__main__':
    ...
    save_path = f'/mnt/sde/deinter_datasets/GCP_backup/deinter_dataset/qtgmc_winter_rst_vertical_median_x2_full_image_size{vertical_size}'
    os.makedirs(save_path, exist_ok=True)
    
    unimatch = Unimatch(gt_root=folder_path)
    verticalmedian = VerticalMedian()
    
    process_images_in_folder_twice(folder_path, save_path, unimatch, verticalmedian, threshold, magnitude,vertical_size)
